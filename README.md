...
실측 결과와 목적마다 설명 카드(무엇이 바뀌고, 켠 뒤 무엇을 겪고, 어떻게 되돌리는지)를 보인 뒤 적용할
목적을 고르게 하고, 파일을 쓰거나 GitHub 설정을 바꾸기 전에 다시 확인받습니다. 파일 변경은 기본 브랜치에
직접 커밋하지 않고 `repo-setup/<날짜>` 브랜치의 PR 한 개로 올립니다. 커밋하지 않은 변경이 있으면 시작하지
않습니다.
