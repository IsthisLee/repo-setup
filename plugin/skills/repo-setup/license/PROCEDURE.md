...
```text
라이선스 (license)
1. 무엇: 저장소 루트의 LICENSE 와 매니페스트(package.json 등)의 license 필드가 같은 라이선스를
   말하게 한다. 라이선스는 사람이 고르고, 전문은 GitHub 라이선스 API 에서 받는다.
2. 이유: 이 저장소는 {공개 여부}이고, LICENSE 는 {LICENSE 상태}, 매니페스트는 {매니페스트 license}다.
3. 바뀌는 것: LICENSE 가 없을 때만 새로 만든다. 매니페스트는 license 필드만 고친다.
   다른 사람: 저작권자 이름과 연도가 LICENSE 에 적혀 공개된다.
4. 겪는 일: 이 저장소의 코드를 받아 쓰는 사람은 고른 라이선스의 조건을 따른다. 저작권자 이름이
   개인 패턴에 있으면 개인 정보 가드가 이 커밋을 막으므로, 공개할 이름을 다시 정한다.
5. 감수할 것: 법적 결정이다. 이 도구는 선택지와 차이만 보이고 대신 고르지 않는다.
6. 되돌리기: 되돌리는 PR 로 파일을 되돌린다. 이미 받아 간 사람의 사본에는 그때의 LICENSE 가 남는다.
7. 건너뛰면: 라이선스가 없으면 저작권 전부 보유가 기본이라, 공개 저장소여도 남이 코드를 쓸 수 없다.
```
...
라이선스는 회사의 결정이지 개인의 결정이 아니다. **팀이 정한 라이선스와 저작권자를 사용자가 알려 준
때만** 파일을 만들어, 확인을 받은 뒤 세팅 PR 에 넣는다. 병합은 팀이 정한다. 알려 주지 않았으면 파일을
만들지 말고 현재 상태와 어긋난 곳만 보고한다. `check-license.sh` 를 돌려 대조하는 것까지는 해도 된다.
읽기만 하기 때문이다.
