...
| `repo-secure` | 호스트가 GitHub 다 | 권한이 없으면 제안만 한다 | 4 | `repo-ci` |
| `repo-contrib` | 공개이고 기여를 받거나 팀 저장소다 | 개인 비공개 | 5 | `repo-ci` |
...
**`repo-contrib` 이 맨 뒤다.** 브랜치 보호에 CI 상태 검사를 필수로 걸려면 CI 가 먼저 있어야
한다. 없는 검사 이름을 필수로 걸면 영원히 통과하지 않는다.

**`repo-ci` 가 `repo-secure` 보다 앞이다.** `repo-ci` 가 워크플로 파일을 만들고
`repo-secure` 가 그 파일의 액션 고정과 권한을 고친다. 뒤집으면 뒤에 만들어진 워크플로가
고정되지 않은 채로 남는다.
...
들어간다. 필요하면 좁은 스킬을 새로 만드는 것이 순서다.
